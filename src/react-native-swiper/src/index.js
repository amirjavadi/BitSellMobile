...
export default class extends Component {
  ...
  renderPrevButton = () => {
    ...
    return (
      <TouchableOpacity onPress={() => this.state.SMSIndex === 2 ? button !== null && this.sendSMS() : this.verificationTest()}>
        <View>
          {button}
        </View>
      </TouchableOpacity>
  ...
  sendSMS() {
    ...
    if (this.props.shopImageUri !== '') {
      if (this.props.nikeName.length > 4) {
        if (regex.test(this.props.mobile)) {
          //send sms
          this.scrollBy(-1)
        } else {
          this.props.callbackResult('sms');
        }
      } else {
        this.props.callbackResult('nikeName');
      }
    } else {
      this.props.callbackResult('shopImageUri');
    }
  };

  verificationTest() {
    if (this.props.verificationCode === '1234') {
      this.scrollBy(-1);
      this.setState({SMSIndex: 0});
      this.props.newIndex(0);
    } else {
      this.props.callbackResult('code');
    }
  };
  ...
  renderScrollView = pages => {
    if (Platform.OS === 'ios') {
      return (
        <FlatList ref={(view) => {this.flatListRe = view}}
          {...this.props}
