...
export default class Product extends React.Component{
  ...
  async componentDidMount(){
    ...
    let products = [];
    axios.get(api.url + '/api/Product/GetProductsWithExtendFields?categoryId=' + this.props.categoryItem.idCat + '&brandId=' + this.props.brandId, {headers: headers})
      .then((response) => {
  ...
  check(item){
    if (Actions.currentScene === 'singleProduct'){
      ...
      Actions.lightbox({productImage: item.imageIndex, productTitle: item.titleProduct, productId: item.id, min: item.min, max: item.max, brandTitle: this.props.brandTitle})
    }
  }
  ...
  render(){
    return(
      <Container>
        ...
        />
        <Header name={this.props.categoryItem.titleCat + ' ' + this.props.brandTitle}/>
